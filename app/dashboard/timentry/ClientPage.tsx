...
export default function TimeEntryClientPage() {
  ...
  const handleAddOrEditActivity = async (entry: any) => {
    ...
    if (!eid) {
      ...
      const r = await fetch("/api/timentry", {
        method: "POST",
    ...
    const payload = {
      timeEntryId: eid,
      activity:
        entry.activity || entry.activityDesc?.split(" ")[0] || "Activity",
    ...
    if (editingId) {
      ...
      await fetch("/api/timentry/activity", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
    ...
    async (info: ReturnType<typeof getShiftInfo>) => {
      ...
      try {
        const res = await fetch(
          `/api/timentry?shiftDate=${info.shiftDate}&shiftType=${info.shiftType}`,
          { cache: "no-store" },
        );
            ...
            if (e.startTime && e.endTime) {
              ...
              if (startParts.length === 2 && endParts.length === 2) {
                const sh = Number(startParts[0]),
                  sm = Number(startParts[1]);
                const eh = Number(endParts[0]),
                  em = Number(endParts[1]);
              ...
              <div className="flex justify-end gap-2 mt-4">
                ...
                <Button
                  ...
                  onPress={async () => {
                    ...
                    const res = await fetch("/api/timentry", {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify(payload),
                    });
            ...
            {(onClose) => (
              <div className="p-4">
                ...
                <p className="text-sm text-default-500 mb-4">
                  Apakah yakin menutup sesi timesheet ini?
                </p>
                <div className="flex gap-2 justify-end">
                  ...
                  <Button
                    ...
                    onPress={async () => {
                      if (openEntryId) {
                        await fetch(`/api/timentry/${openEntryId}`, {
                          method: "PATCH",
                          headers: { "Content-Type": "application/json" },
