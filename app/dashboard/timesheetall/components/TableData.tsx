...
export default function TableDatas({
  ...
  const handleApprove = async (entry: TimesheetEntry) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setLocalData((prev) =>
          prev.map((e) =>
            (e.timeEntryId ?? e.id) === timeEntryId
              ? { ...e, approvedBy: currentUserName }
              : e,
          ),
        );
      } else {
  ...
  const [localData, setLocalData] =
    React.useState<TimesheetEntry[]>(timesheetData);
  ...
  const handleDelete = async (entry: TimesheetEntry) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setLocalData((prev) =>
          prev.filter((e) => (e.timeEntryId ?? e.id) !== timeEntryId),
        );
        setDeletingId(null);
      } else {
        ...
        <div className="flex items-center gap-3 flex-1 justify-start self-start">
          ...
          <div className="flex flex-col flex-1 text-left">
            ...
            <p className="text-small text-default-600">
              Aktivitas Timesheet Semua User
            </p>
          </div>
        </div>
                ...
                {pagedData.map(([tid, activities], idx) => {
                  ...
                  return (
                    <>
                      <TableRow
                        key={tid}
                        className={`cursor-pointer transition-colors ${selectedRow === tid ? "bg-gray-100 dark:bg-[#27272a]" : ""}`}
                        onClick={() =>
                          setSelectedRow(selectedRow === tid ? null : tid)
                        }
                      >
                        ...
                        <TableCell className="text-center">
                          {first.startTime
                            ? new Date(first.startTime).toLocaleTimeString(
                                "id-ID",
                                {
                                  hour: "2-digit",
                                  minute: "2-digit",
                        ...
                        <TableCell className="text-center">
                          {first.endTime
                            ? new Date(first.endTime).toLocaleTimeString(
                                "id-ID",
                                {
                                  hour: "2-digit",
                                  minute: "2-digit",
                          ...
                          {first.approvedBy == null ? (
                            <>
                              <Button
                                ...
                                variant="solid"
                                onPress={() =>
                                  handleApprove({ ...first, timeEntryId: tid })
                                }
                              >
                              ...
                              <Button
                                ...
                                variant="solid"
                                onPress={() =>
                                  handleDelete({ ...first, timeEntryId: tid })
                                }
                              >
                            ...
                            <div>
                              ...
                              <span className="text-xs text-gray-900 font-semibold dark:text-gray-100">
                                {first.approvedBy}{" "}
                              </span>
                            </div>
                          )}
                      ...
                      {selectedRow === tid && (
                        <TableRow>
                          <TableCell
                            className="bg-default-100 dark:bg-zinc-800 border-none border-default-300 dark:border-zinc-700 rounded-b-lg shadow-lg"
                            colSpan={8}
                          >
                            ...
                            <div className="flex flex-col gap-2 w-full py-2 pr-2">
                              {[...activities]
                                ...
                                .map((a) => (
                                  <div
                                    key={a.id}
                                    className="flex flex-col md:flex-row md:items-center justify-between gap-2 bg-default-50 dark:bg-zinc-900 border border-default-200 dark:border-zinc-700 rounded px-3 py-2"
                                  >
                                    <div className="flex flex-col md:flex-row md:items-center gap-2 flex-1">
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs text-default-600 dark:text-gray-400">
                                          {a.startTime
                                            ? new Date(
                                                a.startTime,
                                      ...
                                      >
                                        <ModalContent>
                                          <div className="p-6 flex flex-col items-center">
                                            ...
                                            <div className="mb-4 text-center text-default-600">
                                              Yakin ingin menghapus activity
                                              ini?
                                            </div>
                                              ...
                                              <Button
                                                ...
                                                onPress={async () => {
                                                  ...
                                                  try {
                                                    ...
                                                    await fetch(
                                                      `/api/timesheetall/activity/${deleteActivity.id}`,
                                                      { method: "DELETE" },
                                                    );
