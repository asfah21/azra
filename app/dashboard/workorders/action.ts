...
async function getNextBreakdownNumber(prefix: string) {
  try {
    ...
    const lastNumber = parseInt(lastBreakdown.breakdownNumber.split('-')[1]);
...
export async function createBreakdown(prevState: any, formData: FormData) {
  try {
    ...
    // Validation
    if (!description || !breakdownTime || isNaN(workingHours) || !unitId || !reportedById) {
    ...
    const prefix = (user?.role === "super_admin" || user?.role === "admin_elec") ? "WOIT" : "WO";
