          ...
          <>
            ...
            <ModalBody className="pb-6">
              <div className="space-y-6">
                  ...
                  <CardBody>
                    <div className="flex flex-col gap-3">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-default-500 w-32">Status</span>
                        <Chip
                          color={getStatusColor(breakdown.status) as any}
                          variant="flat"
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-default-500 w-32">Dilaporkan</span>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-default-500 w-32">Working Hours</span>
                        <span className="text-default-900">{breakdown.workingHours} jam</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-default-500 w-32">Reported By</span>
                        <User
                          classNames={{
                      ...
                      {breakdown.rfuReport && (
                        <>
                          <div className="flex items-center gap-2">
                            <span className="text-sm text-default-500 w-32">RFU by</span>
                            <User
                              classNames={{
                                name: "text-default-900",
                                description: "text-default-600",
                              }}
                              description={breakdown.rfuReport.resolvedBy?.email || "-"}
                              name={breakdown.rfuReport.resolvedBy?.name || "-"}
                            />
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-sm text-default-500 w-32">RFU Date</span>
                            <span className="text-default-900">
                ...
                <Card>
                  <CardBody>
                    <h2 className="text-lg font-semibold mb-3">Unit Information</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <Card>
                      <CardBody>
                        <h2 className="text-lg font-semibold mb-3">Deskripsi Kerusakan</h2>
                        <p className="text-default-600">{breakdown.description}</p>
                      </CardBody>
                    </Card>
                    <Card>
                      <CardBody>
                        <h2 className="text-lg font-semibold mb-3">Komponen Terkait</h2>
                    ...
                    <Card>
                      <CardBody>
                        ...
                        <div className="space-y-3">
                          <div>
                            <p className="text-sm font-medium text-default-500">Solusi</p>
                            <p className="text-default-900">{breakdown.rfuReport.solution}</p>
                          </div>
                          {breakdown.rfuReport.actions && breakdown.rfuReport.actions.length > 0 && (
                            <div>
                              <p className="text-sm font-medium text-default-500 mb-2">Tindakan</p>
                              <div className="space-y-2">
                                {breakdown.rfuReport.actions.map((action, index) => (
                                  <div key={action.id} className="border rounded-lg p-3 bg-default-50">
                                    <div className="flex items-center gap-2 mb-2">
                                      <Chip color="primary" variant="flat" size="sm">
                                        Action #{index + 1}
                                      </Chip>
                                      <span className="text-xs text-default-500">
                                        {new Date(action.actionTime).toLocaleString("id-ID", {
                                          dateStyle: "short",
                                          timeStyle: "short",
                                        })}
                                      </span>
                                    </div>
                                    <p className="font-medium text-default-900 mb-1">{action.action}</p>
                                    {action.description && (
