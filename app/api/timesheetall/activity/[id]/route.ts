...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
	try {
		...
		const parent = await prisma.timeEntry.findUnique({ where: { id: activity.timeEntryId } });
		if (parent?.approvedBy) {
			return NextResponse.json({ error: 'Cannot delete activity after approval' }, { status: 403 });
		}
