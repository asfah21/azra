...
const createSchema = z.object({
  ...
  location: z.string().optional().or(z.literal('').transform(() => undefined)),
...
export async function POST(request: Request) {
  ...
  if (!data.activities || data.activities.length === 0) {
    if (!data.startTime || !data.endTime) {
      return NextResponse.json({ error: 'startTime and endTime are required when activities array is not provided' }, { status: 400 });
    }

    startDt = buildDateTime(data.shiftDate, data.startTime, data.shiftType) ?? undefined;
    endDt = buildDateTime(data.shiftDate, data.endTime, data.shiftType) ?? undefined;
    if (!startDt || !endDt) {
  ...
  if (shiftType === 'DAY') {
    ...
    if (!startDt || !endDt || startDt.getTime() < min.getTime() || endDt.getTime() > max.getTime()) {
      return NextResponse.json({ error: 'Time outside DAY shift (06-18)' }, { status: 400 });
    }
  } else {
    // NIGHT: window 18:00 shiftDate -> 06:00 shiftDate+1
    const nStart = (buildDateTime(data.shiftDate, '18:00', 'NIGHT') ?? new Date(0));
    const nEnd = (buildDateTime(data.shiftDate, '06:00', 'NIGHT') ?? new Date(0)); // this becomes next day 06:00
    if (!startDt || !endDt || startDt.getTime() < nStart.getTime() || endDt.getTime() > nEnd.getTime()) {
  ...
  try {
    ...
    if (data.activities && data.activities.length > 0) {
      for (const a of data.activities) {
        const aStart = buildDateTime(data.shiftDate, a.startTime, data.shiftType);
    ...
    const withActivities = await (prisma as any).timeEntry.findUnique({ where: { id: created.id }, include: { activities: true } });
    ...
    return NextResponse.json({
      id: created.id,
      shiftDate: formatDateInTimeZoneISO(created.shiftDate ?? new Date(data.shiftDate + 'T00:00:00.000Z'), 'Asia/Singapore'),
