...
export async function POST(req: NextRequest) {
  try {
    ...
    if (file.size > 1024 * 1024) {
      return NextResponse.json({ success: false, message: "File size too large (max 1MB)" }, { status: 400 });
    }
    ...
    // Bentuk URL publik
    const base = process.env.MINIO_PUBLIC_BASEURL || `${process.env.MINIO_USE_SSL === "true" ? "https" : "http"}://${process.env.MINIO_ENDPOINT}:${process.env.MINIO_PORT}`;
    ...
    return NextResponse.json({ success: true, message: "Signature uploaded successfully", signatureUrl });
  } catch (error) {
