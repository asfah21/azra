...
import { Shuffle, SortAsc, RefreshCcw, List, Trophy, XCircle, ChevronRight, ChevronLeft, Settings, Hash, RotateCcw } from "lucide-react";
...
export default function WheelClientPage() {
  ...
  const [forcedWinnersMap, setForcedWinnersMap] = useState<Record<number, string>>({});
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('wheelForcedWinnersMap', JSON.stringify(forcedWinnersMap));
    }
  }, [forcedWinnersMap]);
  ...
  const playSpinSound = (duration: number) => {
    ...
    const tick = () => {
      ...
      const nextInterval = startInterval + (endInterval - startInterval) * Math.pow(progress, 2);
  ...
  const handleSpin = async () => {
    ...
    const forcedIndex = names.findIndex(n => n.toLowerCase() === nextForcedWinner.trim().toLowerCase());
  ...
  const WheelSegment = ({ index, total, text }: { index: number, total: number, text: string }) => {
    ...
    return (
      <g>
        <path d={pathData} fill={COLORS[index % COLORS.length]} stroke="none" strokeWidth={total > 200 ? "0" : "0.005"} />
        {showText && (
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[calc(100vh-64px)] bg-[#1e2330] text-gray-100 overflow-hidden group">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 lg:p-10 relative overflow-hidden">
        ...
        >
            {isPanelVisible ? <ChevronRight size={24} /> : <ChevronLeft size={24} />}
        </button>
        ...
        <div className="relative w-full max-w-[600px] aspect-square flex items-center justify-center">
            ...
            >
                <svg viewBox="-1 -1 2 2" className="w-full h-full border-none">
                    {names.map((name, i) => (
                        <WheelSegment key={i} index={i} total={names.length} text={name} />
                    ))}
                </svg>
            </motion.div>
            ...
            <div 
                ...
                onClick={handleSpin}
            >
                <span className="text-gray-800 font-black text-lg select-none tracking-wide">SPIN</span>
            </div>
        </div>
            ...
            <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                ...
                >
                    ...
                    <div className="flex gap-3 justify-center">
                        ...
                        <button 
                            onClick={() => {
                                const newText = textInput.split('\n').filter(n => n.trim() !== winner).join('\n');
        ...
        <div className="w-[400px] h-full flex flex-col">
            ...
            <div className="flex border-b border-white/10">
                ...
                >
                    ...
                    <span className="bg-white/10 text-xs py-0.5 px-2 rounded-full">{names.length}</span>
                    {activeTab === 'entries' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-500" />}
                </button>
                <button 
                ...
                >
                    ...
                    <span className="bg-white/10 text-xs py-0.5 px-2 rounded-full">{results.length}</span>
                    {activeTab === 'results' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-500" />}
                </button>
            </div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        {results.length === 0 ? (
                            ...
                            <div className="space-y-1">
                                {results.map((r, i) => (
                                    <div key={i} className="flex items-center gap-3 p-3 bg-[#3f465a]/30 rounded-lg border border-white/5 animate-in fade-in slide-in-from-right-4 duration-300">
                                        ...
                                        <span className="ml-auto text-xs text-gray-500">{new Date().toLocaleTimeString()}</span>
                                    </div>
                                ))}
                            </div>
                        )}
