...
import { X, Settings, Trash2, UserCheck } from "lucide-react";
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#1b1f2b] p-4 rounded-xl border border-white/5 disabled:opacity-50">
              <label
                className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1"
                htmlFor="total-prizes"
              >
                Total Prizes to Give
              </label>
              <input
                ...
                value={localTotal}
                onChange={(e) =>
                  setLocalTotal(Math.max(1, parseInt(e.target.value) || 1))
                }
              />
            </div>
          ...
          <div className="space-y-4">
            <h4 className="flex items-center gap-2 text-sm font-bold text-gray-400 uppercase tracking-wide">
              <UserCheck className="text-green-500" size={16} /> Assign Winners
              to Slots
            </h4>
